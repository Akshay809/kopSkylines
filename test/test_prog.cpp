...
int main(int argc, char const *argv[])
{
	...
	// cout << "Number of objects read: " << data.size() << endl;
	// for (vector<DataObject*>::iterator itr = data.begin(); itr != data.end(); ++itr) {
	// 	(*itr)->printDataObject();
	// }
	...
	while(H.isNotEmpty()) {
		...
		if(nonSkylineIds.find(U->getID())!=nonSkylineIds.end()) {
			cout << "Found in Non-Skyline ids. Exiting." << endl;
			continue;
		}
		// cout << "u here" << endl;
		// u.printDataInstance();
		...
		// cout << "Layer size.. " << at << " " << layerOfU[at].size() << endl;
